 ...
 *
 * If an element exists with the same name plus "HTML" as prefix, the custom element will be defined as an extension of the existing element. For example, if DialogElement is passed as a custom element, HTMLDialogElement will be found to exist and DialogElement will be defined as extending HTMLDialogElement.
 *
 * @example
 * ```javascript
 ...
 */
export function defineCustomElements(elements: Record<string, Constructor<HTMLElement>>, prefix = 'gm-', registry = window.customElements, context = window): void {
  Object
    .entries(elements)
